...
int timPg(std::string dumb){	
	...
	time_t upt = static_cast<time_t>(info.uptime);
	ltm = localtime(&upt);
	utm <<std::setw(2)<<std::setfill('0')<<ltm->tm_mon<<'M'
		<<std::setw(2)<<std::setfill('0')<<ltm->tm_mday<<'D'
		<<std::setw(2)<<std::setfill('0')<<ltm->tm_hour<<'h'
		<<std::setw(2)<<std::setfill('0')<<ltm->tm_min<<'m'
		<<std::setw(2)<<std::setfill('0')<<ltm->tm_sec<<'s';
	npt.curPos(2, 0);
